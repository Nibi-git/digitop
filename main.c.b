...
unsigned int ProtectTimerOnMaxValue, ProtectTimerOnValue;
...
//volatile unsigned long SummUUResult; // отдается внешнему циклу
...
{
if (UpTreshold > 280) UpTreshold = 280;
...
{
if (ProtectTimerOnMaxValue > 600) ProtectTimerOnMaxValue = 600;
...
if (temp > 0)
  ...
  ZeroSamplesCounter = 0;
    ...
    if ((ZeroSamplesCounter > 20)&&(PlusSamplesCounter > 60)) // полуволна закончилась
      {
      PeakResult = Peak;
...
__enable_interrupt();
  ...
  if (ResultSummReady)//
	...
	if  ((Voltage < DownTreshold) && (Voltage >= FastProtectVoltageTreshold)) 
		{
		if (ProtectTimerOffValue) ProtectTimerOffValue--;
		}
